...
class ResponseGenerator:
    def generate(self, state, use_llm=True):
        deterministic_result = self._compose_report(
            title=state.current_plan.objective,
            tool_outputs=state.tool_outputs,
            trace=state.reasoning_trace,
        )
        state.final_report = deterministic_result

        if not use_llm:
            return deterministic_result

        return self._enhance_with_llm(state, deterministic_result)

    def _compose_report(self, title, tool_outputs, trace):
    ...
    def _enhance_with_llm(self, state, deterministic_result):
        if not is_llm_configured():
            state.add_trace(
                'Write',
                'OpenAI API key was not found, so PIA used the deterministic report writer.',
            )
            return deterministic_result

        state.add_trace(
            'Write',
            'Send verified JSON tool outputs to OpenAI for professional report writing.',
        )
        ...
        try:
            ...
            state.add_trace(
                'Fallback',
                'OpenAI report writing failed, so PIA returned the deterministic report.',
            )
            state.final_report = deterministic_result
            return deterministic_result
